...
{
    /* effective from configured params (interface overrides default) */
				   ...
				   struct prefix *p)
{
	...
	if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
		zlog_debug("R5: %s adj_pacing initialized: mode=NONE dynamic_limit=%u H=%u L=%u",
			   ifp->name, oi->adj_pacing.dynamic_limit,
	...
	if (OSPF_IF_PARAM_CONFIGURED(oi->params, adj_pacing_mode)) {
		...
		oi->adj_pacing.static_limit =
			oi->params->adj_pacing_static_limit;
	}
...
void ospf_if_free(struct ospf_interface *oi)
{
	...
	if (oi->adj_pacing.queue){
		list_delete(&oi->adj_pacing.queue);
		oi->adj_pacing.queue= NULL;
	}
	/*RFC4222/R4 Changes*/
...
static struct ospf_if_params *ospf_new_if_params(void)
{
	...
	/* RFC4222 Rec4 defaults (only take effect when enabled) */
