...
{
	return state == NSM_ExStart || state == NSM_Exchange
	       || state == NSM_Loading;	
}
...
static uint32_t ospf_adj_total_unacked(struct ospf_interface *oi)
{
	...
	if (IS_DEBUG_OSPF(nsm, NSM_EVENTS) && total > 0)
		zlog_debug("R5-DYN: %s total_unacked=%u across %u neighbors",
...
static void ospf_adj_dyn_adjust_timer(struct event *t)
{
	...
	if (new_limit != limit) {
		...
		if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
			zlog_debug("R5-DYN: %s ADJUSTED limit %u->%u U=%u H=%u L=%u elapsed=%" PRIu64 "ms",
				   IF_NAME(oi), limit, new_limit, U, H, L, elapsed_ms);

		/* May allow queued adjacencies to proceed */
		if (new_limit > limit && oi->adj_pacing.in_progress < new_limit)
			if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
				zlog_debug("R5-DYN: %s limit increased, may kick queued adjacencies",
					   IF_NAME(oi));
	}
}
...
static uint16_t ospf_adj_pacing_limit(struct ospf_interface *oi)
{
	...
	if (oi->adj_pacing.mode == OSPF_ADJ_PACING_DYNAMIC) {
		...
		if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
			zlog_debug("R5-DYN: %s returning dynamic_limit=%u",
...
/* RFC4222/R5 changes:Remove a neighbor from the per-interface pacing queue. */
static void ospf_adj_pacing_remove(struct ospf_neighbor *nbr){
	/* Drop neighbor from the queue (if present)*/
	struct ospf_interface *oi = nbr->oi;
	struct listnode *node = NULL;

	if (!oi -> adj_pacing.queue)
		return;
	
	node = listnode_lookup(oi->adj_pacing.queue, nbr);
	if (node){
		list_delete_node(oi->adj_pacing.queue, node);	
	}
}

/* RFC4222/R5 changes: Enqueue a neighbor to wait for an available packing slot */
static void ospf_adj_pacing_enqueue(struct ospf_neighbor *nbr){

	/* Add neighbor to per-interface queue when packing blocks ExStart*/
	struct ospf_interface *oi = nbr->oi;
	if (!ospf_adj_pacing_enabled(oi))
		return;
	if (!oi->adj_pacing.queue){
	...
	if (listnode_lookup(oi->adj_pacing.queue, nbr))
		return;

	listnode_add(oi->adj_pacing.queue, nbr);

}

/* RFC4222/R5 changes: Pop the next queued neighbor for this interface */
static struct ospf_neighbor *ospf_adj_pacing_dequeue(struct ospf_interface *oi)
{
	/* FIFO deque for pending adjacencies on this interface*/
	struct listnode *node= NULL;
	...
	node = listhead(oi->adj_pacing.queue);
	if (!node)
		return NULL;

	nbr = listgetdata(node);
	list_delete_node(oi->adj_pacing.queue, node);
	return nbr;
}

/*RFC4222/R5 changes: start queued adjacencies when a pacing slot opens*/
static void ospf_adj_pacing_kick(struct ospf_interface *oi){
	/* Schedule NSM_Adjok for queued neighbors while slots are availble*/
	struct ospf_neighbor *nbr = NULL;

	while(ospf_adj_pacing_allow(oi)){
...
static int nsm_twoway_received(struct ospf_neighbor *nbr)
{
	...
	if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
		zlog_debug("R5: TwoWay recv nbr=%pI4 adj=%d",
			   &nbr->router_id, adj);
	
	if (adj && !ospf_adj_pacing_allow(nbr->oi)){
		if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
			zlog_debug("R5: throttling nbr=%pI4",
				   &nbr->router_id);
	...
	if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
		zlog_debug("R5: TwoWay -> %s nbr=%pI4",
			   adj ? "ExStart" : "TwoWay",
			   &nbr->router_id);
...
static int nsm_adj_ok(struct ospf_neighbor *nbr)
{
	...
	if (nbr->state == NSM_TwoWay && adj == 1) {
		if (!ospf_adj_pacing_allow(nbr->oi)) {
			if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
				zlog_debug("R5: AdjOK throttling nbr=%pI4",
					   &nbr->router_id);
			ospf_adj_pacing_enqueue(nbr);
			return NSM_TwoWay;
		}
...
static void nsm_change_state(struct ospf_neighbor *nbr, int state)
{
	...
	if (!ospf_adj_in_progress_state(old_state) &&
	    ospf_adj_in_progress_state(state)) {
		oi->adj_pacing.in_progress++;
	} else if (ospf_adj_in_progress_state(old_state) &&
		   !ospf_adj_in_progress_state(state)) {
		if (oi->adj_pacing.in_progress > 0)
			oi->adj_pacing.in_progress--;

		if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
			zlog_debug("R5: %s in-progress=%u limit=%u",
				   IF_NAME(oi),
				   oi->adj_pacing.in_progress,
